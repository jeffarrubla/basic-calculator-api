...
class SubsTest(TestCase):
	""" Test module for subtraction API """

	def test_subs(self):
		# get API response
		response = client.get(reverse('subtraction',kwargs={'a': 4, 'b': 2}))
		# do assertions 
	...
	def test_subs_negative_result(self):
		# get API response
		response = client.get(reverse('subtraction',kwargs={'a':'2','b':'3'}))
		# do assertions
	...
	def test_subs_special_char(self):
		#get API response
		response = client.get(reverse('subtraction',kwargs={'a':'s','b':5}))
		# do assertions
		self.assertEqual(response.json()['Error'],"could not convert string to float: 's'")
		self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
	...
	def test_subs_with_symbol(self):
		# get API response
		response = client.get(reverse('subtraction',kwargs={'a':'-','b':'4'}))
		# do assertions
		self.assertEqual(response.json()['Error'],"could not convert string to float: '-'")
		self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
	...
	def test_sub_with_float(self):
		# get API response
		response = client.get(reverse('subtraction',kwargs={'a':'10.5935','b':'1002.49'}))
		# do assertions
